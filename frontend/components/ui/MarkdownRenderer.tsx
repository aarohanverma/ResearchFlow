...
export interface DecorationHighlight {
  id: string;
  /** The full selection text (``Selection.toString()``). Used as the
   *  primary match when the selection sits inside a single rendered
   *  text node (the common case). */
  text: string;
  /** Per-text-node slices of the selection, captured when the
   *  selection spans multiple rendered text nodes (e.g. across bold /
   *  italic / heading / inline-math / citation chips). Each entry is
   *  exact text from one underlying ``Text`` node, so the renderer
   *  can match it without resorting to a fuzzy LCS search. */
  segments?: string[];
  color: string;
}
 ...
 *
 * Highlight matching is exact-substring-only: the stored ``hl.text``
 * has to appear verbatim inside this rendered text node for it to
 * mark. A previous attempt at "fuzzy" cross-boundary matching ran a
 * longest-common-substring search instead and ended up highlighting
 * every paragraph in the document because short tokens like ``the``
 * or ``of `` matched indiscriminately. Cross-boundary support needs
 * to be done by capturing the selection's text-node anchors at
 * mouseup time and matching each anchor separately (see
 * ``HighlightSearch.tsx``), NOT by widening the matcher here.
 */
function _findMatches(text: string, dec?: MarkdownDecorations): DecorationMatch[] {
  ...
  for (const hl of dec.highlights || []) {
    // Match every supplied anchor (``hl.text`` plus optional
    // ``hl.segments`` for cross-formatting-boundary selections).
    // Each anchor must be at least 3 chars to avoid noise from
    // short common words.
    const anchors: string[] = [];
    if (hl.text && hl.text.length >= 3) anchors.push(hl.text);
    for (const seg of (hl.segments || [])) {
      if (seg && seg.length >= 3 && !anchors.includes(seg)) {
        anchors.push(seg);
      }
    }
    for (const needle of anchors) {
      for (const sp of _exactHighlightSpans(text, needle)) {
        matches.push({
          start: sp.start,
          end: sp.end,
          kind: "highlight",
